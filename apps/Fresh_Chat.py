...
def app():
    ...
    model = load_model('Econet_Fresh-Chat')
    ...
    st.title("Fresh Chat Productivity")
...
""")
    ...
    Conversion  = st.sidebar.number_input('Conversion', min_value=0.00, max_value=1000.00, value=0.00)
    ...
    input_dict = {'AES' : AES, 'Resolved Count' : Resolved, 'Conversion ' : Conversion, 'CSAT' : CSAT}
    ...
    if st.button("Predict"):
        ...
        if AES<98:
            ...
            st.warning(aes1)
        if Resolved <140:
            calls1= "You should improve intaractions to more than 140 daily"
            st.warning(calls1)
        if Conversion >98:
            ath1= "Aim to increase conversion rate to 98%"
            st.warning(ath1)
        if CSAT < 85:
        ...
        st.image('undraw_progressive_app_m9ms.png', width=700)